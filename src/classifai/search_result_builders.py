"""Functions to process and format the results of ChromaDB search to be returned to Flask in response body."""
...
def process_embedding_search_result(
    ...
    include_bridge: boolean
        Parameter to determine if bridge metadata should be added to the resulting ranking - see differences in SOC and SIC vector DB metadata.

    Returns
    -------
    processed_result : list
        list format of Chroma DB vector search results
    """
    processed_result = [
        {
            "input_id": input_id,
            "response": [
                {
                    ...
                    "distance": "{:.3f}".format(
                        query_result["distances"][i][j]
                    ),  # rounding score to 3 decimal places
                    "rank": j + 1,
                }
                for j, description in enumerate(query_result["documents"][i])
            ],
        }
...
def inject_SOC_info(deduplicated_response: dict, add_info: dict):
    """TODO: Use pandas dataframe from Peter to add extra metadata into deduplicated data structure."""
    injectable_data = ""
