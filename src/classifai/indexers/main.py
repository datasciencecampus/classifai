...
class VectorStore:
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.meta_data: dict = meta_data if meta_data is not None else {}
        self.vectors: np.array = None
        self.vector_shape: tuple | list | None = None
        self.num_vectors: int = None
        self.vectoriser_class: str = vectoriser.__class__.__name__
    ...
    def embed(self, text: str | list[str] | None) -> np.ndarray:
        """Converts text into vector embeddings using the vectoriser.

        Args:
            text (str,list): The text or list of text to be converted into vector embeddings.
    ...
    def reverse_search(
        self, query: str | list[str], ids: int | list[int] | None = None, n_results: int = 100
    ) -> pd.DataFrame:
        ...
        Args:
            query (str,list): The text query or list of queries to search for.
            ids (int,list): List of query IDs. Defaults to None.
            n_results (int): Number of top results to return for each query. Default 100.
        ...
        """
        # if the query/ids is a string/int, convert it to a list
    ...
    def search(
        self, query: str | list[str], ids: int | list[int] | None = None, n_results: int = 10, batch_size: int = 8
    ) -> pd.DataFrame:
        ...
        Args:
            query (str,list): The text query or list of queries to search for.
            ids (int,list): List of query IDs. Defaults to None.
            n_results (int): Number of top results to return for each query. Default 10.
            batch_size (int): The batch size for processing queries. Default 8.
    ...
    @classmethod
    def from_filespace(cls, folder_path: str, vectoriser) -> Self:
