...
from classifai.evaluation import SICAssignmentEvaluator, MethodComparison, LabelAccuracy
...
metadata = {
    ...
    "knowledge_base": "sic_5_digit_combined",
...
```
### Measure classifAI accuracy
```{python}
with open("data/eval_set.json", "rb") as f:
    classifai_result = json.load(f)
```
Merge evaluation set with classifai
```{python}
classifai_result_df = flatten_json_to_wide_df(classifai_result, top_k = 2)
combined_df = evaluation_set.merge(classifai_result_df, left_on="bus_id", right_on="input_id", how="inner")
combined_df["score_1"] = 1 - combined_df["distance_1"]
combined_df["score_2"] = 1 - combined_df["distance_2"]

```
```{python}
analyzer = LabelAccuracy(combined_df,
id_col="bus_id",
desc_col="desc_processed",
model_label_cols=["label_1", "label_2"],
model_score_cols=["score_1", "score_2"],
clerical_label_cols=["sic_5d", "updated_sic_5d"])
